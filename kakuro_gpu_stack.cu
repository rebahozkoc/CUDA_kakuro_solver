...
struct mat_iter2{
    ...
    mat_iter2(int **_mat, int _m, int _n) : mat(_mat), m(_m), n(_n){
        curr = COORD(0, 0);
    ...
    // Copy constructor
    mat_iter2(const mat_iter2 &other) {
...
__device__ int mat_iter_init_i(int *mat, int m, int n){
    int curr_i = 0;
    int curr_j = 0;
...
__device__ int mat_iter_init_j (int *mat, int m, int n){
    int curr_i = 0;
    int curr_j = 0;
    if (mat[curr_i * n + curr_j]!= -2){
            do {
...
__device__ int mat_iter_get_next_i(int *mat, int m, int n, int curr_i, int curr_j){
    if (mat[curr_i * n + curr_j]!= -2){
            do {
            if (curr_j + 1 < n){
                curr_j += 1;
...
__device__ int mat_iter_get_next_j(int *mat, int m, int n, int curr_i, int curr_j){
    if (mat[curr_i * n + curr_j]!= -2){
            do {
            if (curr_j + 1 < n){
                curr_j += 1;
...
__device__ void push(Stack* s, int value) {
    int idx = atomicAdd(&(s->top), 1);
    s->array[idx] = value;
}

__device__ int pop(Stack* s) {
    int idx = atomicSub(&(s->top), 1);
    return s->array[idx];
}

__device__ bool empty(Stack* s) {
    return (s->top) == 0;
}
...
__global__ 
    ...
    int *d_sum_lengths, int *d_sum_dirs, int *d_sol_mat, int *d_t_mats, int m, int n, int no_sums, Stack* iter_i_stack, Stack* iter_j_stack, Stack* curr_val_stack, volatile bool *solved){
    
    int iter_i = mat_iter_init_i(d_sol_mat,  m, n);
    int iter_j = mat_iter_init_j(d_sol_mat, m, n);
    int curr_val = 1;
    push(iter_i_stack, iter_i);
    push(iter_i_stack, iter_i);
    push(curr_val_stack, curr_val);
    
    //stack<state2> state_stack;
    //state_stack.push(state2(iter, 1));
    while(!*solved || !empty(iter_i_stack)){
        int iter_i = pop(iter_i_stack);
        int iter_j = pop(iter_j_stack);

        int curr_val = pop(curr_val_stack);
        //state_stack.pop();
        if (curr_val < 10){
        
            // Update the matrix with the new val for iter.curr
            d_sol_mat[iter_i * n + iter_j] = curr_val;
            ...
            */
            if (partial_correctness){
                //state_stack.push(state2(iter, curr_val));
                push(iter_i_stack, iter_i);
                push(iter_j_stack, iter_j);
                push(curr_val_stack, curr_val);
                
                int iter_next_i = mat_iter_get_next_i(d_sol_mat, m, n, iter_i, iter_j);
                int iter_next_j = mat_iter_get_next_j(d_sol_mat, m, n, iter_i, iter_j);

                //temp.set_next();
                if (iter_next_i == -999 || iter_next_j == -999){
                    //printf("end inside");
                    //print_flattened_matrix(d_sol_mat, m, n);
                    *solved = true;
                }
                push(iter_i_stack, iter_next_i);
                push(iter_j_stack, iter_next_j);
                push(curr_val_stack, 1);
            }else{
                curr_val += 1;
                d_sol_mat[iter_i * n + iter_j] = -2;
                push(iter_i_stack, iter_i);
                push(iter_j_stack, iter_j);
                push(curr_val_stack, curr_val);
            }
            ...
            while(!empty(iter_i_stack)){
                int iter_i = pop(iter_i_stack);
                int iter_j = pop(iter_j_stack);

                int curr_val = pop(curr_val_stack);
                //mat_iter2 iter = state_stack.top().iter;
...
struct state2{
    mat_iter2 iter;
    int val;
    state2(mat_iter2 _iter, int _val): iter(_iter), val(_val){}
};


bool solution(int **sol_mat, vector<sum> &sums, int m, int n) {    
    mat_iter2 iter = mat_iter2(sol_mat,  m, n);
    stack<state2> state_stack;
    state_stack.push(state2(iter, 1));
    while(!state_stack.empty()){
        ...
        if (curr_val < 10){
            ...
            if (partial_correctness){
                ...
                state_stack.push(state2(temp, 1));
            }else{
                curr_val += 1;
...
int main(int argc, char **argv){
    ...
    Stack* d_iter_i_stack, *d_iter_j_stack, *d_curr_val_stack ;
    cudaMallocManaged(&d_iter_i_stack, sizeof(Stack));
    cudaMallocManaged(&d_iter_j_stack, sizeof(Stack));
    cudaMallocManaged(&d_curr_val_stack, sizeof(Stack));
    int* d_iter_i_array, *d_iter_j_array, *d_curr_val_array;
    cudaMallocManaged(&d_iter_i_array, m*n *9* sizeof(int));
    cudaMallocManaged(&d_iter_j_array, m*n *9* sizeof(int));
    cudaMallocManaged(&d_curr_val_array, m*n *9* sizeof(int));
    ...
    cudaMemcpy(&(d_iter_i_stack->top), &h_top_i, sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_iter_j_stack->top), &h_top_j, sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_curr_val_stack->top), &h_top_val, sizeof(int), cudaMemcpyHostToDevice);

    // Point the stack array to the allocated memory
    cudaMemcpy(&(d_iter_i_stack->array), &d_iter_i_array, sizeof(int*), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_iter_j_stack->array), &d_iter_j_array, sizeof(int*), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_curr_val_stack->array), &d_curr_val_array, sizeof(int*), cudaMemcpyHostToDevice);

	cudaMallocManaged(&d_sum_starts_x, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_starts_y, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_ends_x, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_ends_y, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_hints, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_lengths, no_sums * sizeof(int));
	cudaMallocManaged(&d_sum_dirs, no_sums * sizeof(int));
	cudaMallocManaged(&d_sol_mat, (m * n) * sizeof(int));
	cudaMallocManaged(&d_t_mats, (m * n * grid_dim * block_dim) * sizeof(int)); // Allocating invidual matrix for each GPU thread
	...
	*solved = false;
	...
	cudaMallocManaged(&d_solved, sizeof(bool));
    ...
    kakuro_kernel<<<grid_dim, block_dim>>>(d_sum_starts_x, d_sum_starts_y, d_sum_ends_x, d_sum_ends_y, d_sum_hints,
										   ...
										   no_sums, d_iter_i_stack, d_iter_j_stack, d_curr_val_stack, d_solved);
	cudaDeviceSynchronize();
    bool result = solution(sol_mat, sums, m, n);
    end = omp_get_wtime(); 
